...
function collectJsonLikeStringsDeep(
  node,
  maxDepth = 8,
  currentDepth = 0,
  results = []
  ...
  if (Array.isArray(node)) {
    for (const item of node) {
      collectJsonLikeStringsDeep(item, maxDepth, currentDepth + 1, results);
    }
    return results;
  }
  ...
  for (const key of Object.keys(node)) {
    collectJsonLikeStringsDeep(node[key], maxDepth, currentDepth + 1, results);
  }
  return results;
}
...
function normalizeOpenAiArticleResponse(responseJson) {
  ...
  const jsonStringCandidates = Array.from(
    new Set(collectJsonLikeStringsDeep(responseJson))
  );
  for (const candidate of jsonStringCandidates) {
    try {
      const parsedCandidate = parseJsonObjectFromText(candidate);
      const found = findArticleLikeObjectDeep(parsedCandidate);
      if (found) {
        return found;
      }
    } catch (error) {
      // Ignore non-JSON string candidates.
    }
  }
...
function buildRepairPrompt(originalPrompt) {
  return (
    `${originalPrompt}\n\n` +
    "## Format correction (strict)\n" +
    "- Do not place bodyHtml at root level.\n" +
    "- languages.en.bodyHtml, languages.tl.bodyHtml, and languages.vi.bodyHtml are required.\n" +
    "- Return one JSON object only with keys: slug, lastUpdated, readTimeMinutes, languages.\n" +
    "- Each language block must include: title, metaDescription, badge, excerpt, image(url, alt), bodyHtml.\n" +
    "- No explanation text. JSON only."
  );
}

async function requestOpenAiResponse(apiKey, model, prompt, timeoutMs) {
...
async function generateWithOpenAi(apiKey, model, prompt, timeoutMs) {
  const firstResponse = await requestOpenAiResponse(apiKey, model, prompt, timeoutMs);
  try {
    return normalizeOpenAiArticleResponse(firstResponse);
  } catch (firstError) {
    console.warn(
      "OpenAI output shape was invalid. Retrying once with stricter structure instructions..."
    );
    const secondResponse = await requestOpenAiResponse(
      apiKey,
      model,
      buildRepairPrompt(prompt),
      timeoutMs
    );

    try {
      return normalizeOpenAiArticleResponse(secondResponse);
    } catch (secondError) {
      throw new Error(
        "OpenAI returned JSON but not article structure (slug/languages.en|tl|vi).\n" +
