...
const USER_AGENT = "dating-japan-image-fetcher/1.0";
...
function buildQueries(topicData, slug, overrideQuery) {
  ...
  return uniqueNonEmpty([
    `${topic} ${keywords.join(" ")} japan dating couple`,
    `${topicId} japan dating culture`,
    `${category} japan romance relationships`,
    "japan couple date lifestyle",
  ]);
}
...
function buildFallbackAlt(photoAlt, slug) {
  ...
  return `Dating scene in Japan (${slug})`;
}
